...
class Agent extends CI_Controller {
	...
	public function quotes_received() {
		$data['quote_data'] = $this->db->query("SELECT tbl_quote_request.*, tbl_user.user_type FROM tbl_quote_request left outer join tbl_user on tbl_user.user_id = tbl_quote_request.added_by  WHERE assigned_agent = '$this->user_id' AND (tbl_quote_request.status IS NULL) ORDER BY created_on DESC")->result_array();
		$this->load->view('admin/common/header');
		$this->load->view('admin/common/menubar');
		...
		if (isset($is_new) OR isset($is_edit)) {
			...
			if (!$this->upload->do_upload($file_element_name)) {
				...
				if ($edit_id) {
					...
					//redirect('admin/agent/edit_quote/'.$edit_id);
				} else {
